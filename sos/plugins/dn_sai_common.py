...
class DN_sai_commonPlugin(Plugin, DebianPlugin):
    ...
    def setup(self):
        ...
        if "hshell" not in tmp[:]:				
            self.add_cmd_output("/opt/dell/os10/bin/hshell -c ps", timeout = 10)
            self.add_cmd_output("/opt/dell/os10/bin/hshell -c 'phy info'", timeout = 10)
        else:
            self.add_alert("hshell is already running! halt hshell and rerun sosreport!")
        self.add_copy_spec("/etc/sonic/init.xml")
