...
static const dn_sai_hostif_pkt_attr_property_t packet_attrs[] = {
    { SAI_HOSTIF_PACKET_TRAP_ID, false, false},
    { SAI_HOSTIF_PACKET_INGRESS_PORT, false, false},
    { SAI_HOSTIF_PACKET_INGRESS_LAG, false, false},
    { SAI_HOSTIF_PACKET_TX_TYPE, true, true},
    { SAI_HOSTIF_PACKET_EGRESS_PORT_OR_LAG, true, false}
};

static const dn_sai_hostif_trap_group_attr_property_t trap_group_attrs[] = {
    { SAI_HOSTIF_TRAP_GROUP_ATTR_ADMIN_STATE, false, true},
    { SAI_HOSTIF_TRAP_GROUP_ATTR_PRIO, true, false},
...
sai_status_t sai_hostintf_init(void)
{
    ...
    if (rc != SAI_STATUS_SUCCESS) {
        ...
        attr_list[attr_count].value.u32 = DN_HOSTIF_DEFAULT_TRAP_GROUP_PRIO;
        attr_count++;
                                   ...
                                   const sai_attribute_t *attr_list)
{
    ...
    *trap_group = (dn_sai_trap_group_node_t *) calloc(1,
    ...
    for(attr_idx = 0; attr_idx < attr_count; attr_idx++) {
        ...
        } else if (SAI_HOSTIF_TRAP_GROUP_ATTR_PRIO == attr_id) {
            (*trap_group)->group_prio = attr_val.u32;
...
static sai_status_t sai_set_hostif_trap(sai_hostif_trap_id_t trapid,
                                        const sai_attribute_t *attr)
{
    sai_status_t rc = SAI_STATUS_FAILURE;
    dn_sai_trap_node_t *trap_node = NULL;
...
static sai_status_t sai_get_hostif_trap(sai_hostif_trap_id_t trapid,
                                        uint_t attr_count,
                                        sai_attribute_t *attr_list)
{
    uint_t attr_idx = 0;
