...
void sai_qos_init_buffer_pool_node(dn_sai_qos_buffer_pool_t  *p_buf_pool_node)
{
    ...
    p_buf_pool_node->threshold_mode = SAI_BUFFER_THRESHOLD_MODE_DYNAMIC;
                                                  ...
                                                  *p_profile_node)
{
    ...
    if(p_pool_node->pool_type == SAI_BUFFER_POOL_INGRESS) {
        if((obj_type == SAI_OBJECT_TYPE_PORT) ||
           (obj_type == SAI_OBJECT_TYPE_PRIORITY_GROUP)) {
            return true;
        }
    }
                                                        ...
                                                        const sai_attribute_t *attr)
{
    ...
    if (p_profile_node->profile_th_enable) {
        if ((p_profile_node->threshold_mode == SAI_BUFFER_THRESHOLD_MODE_STATIC) &&
                (attr->id == SAI_BUFFER_PROFILE_ATTR_SHARED_STATIC_TH)) {
            return true;
        }

        if ((p_profile_node->threshold_mode == SAI_BUFFER_THRESHOLD_MODE_DYNAMIC) &&
                (attr->id == SAI_BUFFER_PROFILE_ATTR_SHARED_DYNAMIC_TH)) {
            return true;
        }
    } else {
        if ((p_pool_node->threshold_mode == SAI_BUFFER_THRESHOLD_MODE_STATIC) &&
                (attr->id == SAI_BUFFER_PROFILE_ATTR_SHARED_STATIC_TH)) {
            return true;
        }

        if ((p_pool_node->threshold_mode == SAI_BUFFER_THRESHOLD_MODE_DYNAMIC) &&
                (attr->id == SAI_BUFFER_PROFILE_ATTR_SHARED_DYNAMIC_TH)) {
            return true;
        }
    }
                                                                 ...
                                                                 const sai_attribute_t *attr_list)
{
    ...
    sai_buffer_threshold_mode_t buffer_profile_th_mode;
    uint_t pool_idx = 0;
    ...
    if (!profile_th_setting) {
        ...
        } else if (p_pool_node->threshold_mode == SAI_BUFFER_THRESHOLD_MODE_DYNAMIC) {
            if(!is_dynamic) {
                return SAI_STATUS_MANDATORY_ATTRIBUTE_MISSING;
            }
        }
    } else {
        if (buffer_profile_th_mode == SAI_BUFFER_THRESHOLD_MODE_STATIC){
            if(!is_static) {
                return SAI_STATUS_MANDATORY_ATTRIBUTE_MISSING;
            }

        } else if (buffer_profile_th_mode == SAI_BUFFER_THRESHOLD_MODE_DYNAMIC) {
            if(!is_dynamic) {
                return SAI_STATUS_MANDATORY_ATTRIBUTE_MISSING;
            }
        }
                                                            ...
                                                            *attr_list)
{
    ...
    if ((pfc_th_present) &&
        (p_pool_node->pool_type == SAI_BUFFER_POOL_EGRESS)) {
