...
{
    p_sched_node->sched_algo = SAI_SCHEDULING_WRR;
...
static void sai_qos_scheduler_node_dlft_init (dn_sai_qos_scheduler_t *p_sched_node)
{
    memset (p_sched_node, 0, sizeof(dn_sai_qos_scheduler_t));
    p_sched_node->sched_algo = SAI_SCHEDULING_WRR;
    p_sched_node->weight = SAI_QOS_SCHEDULER_DEFAULT_WEIGHT;
    p_sched_node->shape_type = SAI_QOS_SCHEDULER_DEFAULT_SHAPE_TYPE;
}
                                         ...
                                         const sai_attribute_t *p_attr)
{
    ...
    dn_sai_qos_scheduler_t    *p_sched_node = NULL;
    dn_sai_qos_scheduler_t    delf_sched_node;
    ...
    if (sched_id != SAI_NULL_OBJECT_ID) {
        ...
        p_sched_node = &delf_sched_node;
        sai_qos_scheduler_node_dlft_init (p_sched_node);
    }
                                         ...
                                         const sai_attribute_t *p_attr)
{
    ...
    sai_rc = sai_scheduler_npu_api_get()->scheduler_set (sg_id, NULL, p_sched_node);
...
sai_status_t sai_qos_create_default_scheduler(sai_object_id_t *default_sched_id)
{
    ...
    attr[attr_count].value.s32 = SAI_SCHEDULING_WRR;
