                                              ...
                                              uint_t *p_flags)
{
    ...
    if ((is_create_req) && (!sai_fib_is_neighbor_dest_mac_attr_set (*p_flags))) {
                                    ...
                                    sai_fib_nh_key_t *p_nh_key)
{
    ...
    p_nh_key->nh_type  = SAI_NEXT_HOP_IP;
...
static sai_status_t sai_fib_neighbor_port_id_resolve (sai_fib_nh_t *p_neighbor)
{
    ...
    if (p_rif->type == SAI_ROUTER_INTERFACE_TYPE_PORT) {
        ...
        if (status != SAI_STATUS_SUCCESS) {
            ...
            SAI_NEIGHBOR_LOG_ERR ("Failure to get port id for Neighbor from "
                                  "L2 FDB : %d.", status);
...
static sai_status_t sai_fib_neighbor_mac_entry_remove (sai_fib_nh_t *p_neighbor)
{
    ...
    if (p_mac_entry == NULL) {
        SAI_NEIGHBOR_LOG_ERR ("Neighbor MAC entry node not found.");
                                                  ...
                                                  sai_fib_nh_t **node)
{
    ...
    p_nh_node = sai_fib_ip_next_hop_node_get (nh_info->key.rif_id,
                                    ...
                                    const sai_attribute_t *attr_list)
{
    ...
    if (status == SAI_STATUS_SUCCESS) {
        ...
        SAI_NEIGHBOR_LOG_ERR ("SAI Neighbor entry creation failed.");
                                ...
                                const sai_neighbor_entry_t *neighbor_entry)
{
    ...
    do {
        ...
        p_nh_node = sai_fib_ip_next_hop_node_get (neighbor_entry->rif_id,
                                                  p_ip_addr);
