...
static sai_status_t sai_port_routing_mode_set (sai_object_id_t sai_port_id)
{
    ...
    do {
        ...
        if (fwd_mode == SAI_PORT_FWD_MODE_SWITCHING) {
            SAI_RIF_LOG_ERR ("Port 0x%"PRIx64" is in switching mode", sai_port_id);
...

{
    ...
    sai_rc = sai_port_list_routing_mode_update (port_id_list, is_add);
    ...
    do {
        ...
        if (p_rif_node->attachment.port_id != lag_id) {
            SAI_RIF_LOG_ERR ("RIF attachment Id 0x%"PRIx64" is not same as the "
                             "LAG Id 0x%"PRIx64".",
        ...
        sai_rc = sai_rif_npu_api_get()->rif_lag_member_update (p_rif_node,
                                                               ...
                                                               is_add);
        if (sai_rc != SAI_STATUS_SUCCESS) {
            SAI_RIF_LOG_ERR ("Failed to add LAG member for Router Interface "
