...
static sai_status_t sai_l2_add_ports_to_lag(sai_object_id_t lag_id,
                                     const sai_object_list_t *lag_port_list)
{
    unsigned int num_port = 0;
    unsigned int tmp_port = 0;
    ...
    do {
        ...
        for(num_port = 0; num_port < lag_port_list->count; num_port++) {
            if(sai_is_port_part_of_different_lag(lag_id, lag_port_list->list[num_port])) {
                SAI_LAG_LOG_ERR("Invalid port 0x%"PRIx64" to lag 0x%"PRIx64"",
                                lag_id, lag_port_list->list[num_port]);
                ret_val = SAI_STATUS_INVALID_PORT_MEMBER;
                break;
            }
            sai_port_forward_mode_info(lag_port_list->list[num_port],
                                       &fwd_mode, false);
            if(fwd_mode == SAI_PORT_FWD_MODE_ROUTING) {
                SAI_LAG_LOG_ERR("port 0x%"PRIx64" in routing mode. \
                                Cannot add to lag:0x%"PRIx64"",
                                lag_port_list->list[num_port], lag_id);
            ...
            if(!sai_is_port_lag_member(lag_id, lag_port_list->list[num_port])) {
                ...
                /*
                 * sai_l2_add_ports_to_lag() will be obsoleted. The member
                 * addition API is changed, such that they can be added
                 * one by one. Each member port will be assigned an
                 * object id (member_id) by the NPU layer. However the old
                 * implementation (port list addition) does not require
    ...
    if(ret_val == SAI_STATUS_SUCCESS) {
        ...
        non_dup_port_list.list  = non_dup_list;
        non_dup_port_list.count = num_nodes;
        member_id_list.list     = member_ids;
        member_id_list.count    = num_nodes;
    ...
    if(ret_val == SAI_STATUS_SUCCESS) {
        ret_val = sai_lag_rif_notify (lag_id, SAI_LAG_OPER_ADD_PORTS,
                                     (const sai_object_list_t *)&non_dup_port_list);
        if(ret_val !=SAI_STATUS_SUCCESS) {
            sai_lag_npu_api_get()->remove_ports_from_lag(lag_id,
                                          (const sai_object_list_t *)&non_dup_port_list);
        }
    }
...
static sai_status_t sai_l2_remove_ports_from_lag(sai_object_id_t lag_id,
                                                 const sai_object_list_t *lag_port_list)
{
    unsigned int num_port = 0;
    unsigned int invalid_idx = 0;
    ...
    for(num_port = 0; num_port < lag_port_list->count; num_port++) {
        ...
        if(!sai_is_port_lag_member(lag_id, lag_port_list->list[num_port])) {
            ...
            SAI_LAG_LOG_WARN("Unable to find port 0x%"PRIx64" on lag:0x%"PRIx64"",
                             lag_port_list->list[num_port], lag_id);
            sai_lag_unlock();
            return SAI_STATUS_INVALID_PORT_MEMBER;
        }
    }
    ret_val = sai_lag_npu_api_get()->remove_ports_from_lag(lag_id, lag_port_list);
    if(ret_val == SAI_STATUS_SUCCESS) {
        ret_val = sai_lag_rif_notify (lag_id, SAI_LAG_OPER_DEL_PORTS, lag_port_list);
        if(ret_val !=SAI_STATUS_SUCCESS) {
            sai_object_list_t member_id_list;
            sai_object_id_t   member_ids [lag_port_list->count];

            member_id_list.list  = member_ids;
            member_id_list.count = lag_port_list->count;

            sai_lag_npu_api_get()->add_ports_to_lag(lag_id, lag_port_list,
                                                    &member_id_list);
        }
    }
    ...
    for(num_port = 0; num_port < lag_port_list->count; num_port++) {
        sai_lag_port_node_remove (lag_id, lag_port_list->list[num_port]);
    }
    sai_lag_unlock();
...
static sai_status_t sai_lag_port_validate(sai_object_id_t lag_id, sai_object_id_t port_id)
{
    if(!sai_is_obj_id_lag(lag_id)) {
        SAI_LAG_LOG_ERR("Invalid LAG object type");
        return SAI_STATUS_INVALID_OBJECT_TYPE;
    }
    if(!sai_is_obj_id_port(port_id)) {
        SAI_LAG_LOG_ERR("Invalid Port object type");
        return SAI_STATUS_INVALID_OBJECT_TYPE;
    }
    if(!sai_is_port_valid(port_id)) {
        SAI_LAG_LOG_ERR("Invalid port ID");
...
static sai_status_t sai_l2_lag_port_mode_set(sai_object_id_t lag_id, sai_object_id_t port_id,
                                             sai_lag_port_mode_t lag_port_mode)
{
    sai_status_t ret_val;

    SAI_LAG_LOG_TRACE ("Setting mode (%d) for port 0x%"PRIx64" on "
                       "lag:0x%"PRIx64"", lag_port_mode, port_id, lag_id);
    sai_lag_lock();

    ret_val = sai_lag_port_validate(lag_id, port_id);
    if(ret_val != SAI_STATUS_SUCCESS) {
        sai_lag_unlock();
        return ret_val;
    }
    ret_val = sai_lag_npu_api_get()->lag_port_mode_set(lag_id, port_id, lag_port_mode);
...
static sai_status_t sai_l2_lag_port_mode_get(sai_object_id_t lag_id, sai_object_id_t port_id,
                                             sai_lag_port_mode_t *lag_port_mode)
{
    sai_status_t ret_val;

    if(lag_port_mode == NULL) {
        SAI_LAG_LOG_ERR("Mode is NULL");
        STD_ASSERT(0);
        return SAI_STATUS_INVALID_PARAMETER;
    }
    sai_lag_lock();
    ...
    ret_val = sai_lag_npu_api_get()->lag_port_mode_get(lag_id, port_id, lag_port_mode);
...
static sai_lag_api_t sai_lag_method_table =
{
    ...
    sai_l2_add_ports_to_lag,
    sai_l2_remove_ports_from_lag,
    sai_l2_lag_port_mode_set,
    sai_l2_lag_port_mode_get,
