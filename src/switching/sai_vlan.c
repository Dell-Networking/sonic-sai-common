...
static sai_status_t sai_l2_remove_all_vlans(void)
{
    sai_vlan_id_t vlan_id = SAI_MIN_VLAN_TAG_ID;

    sai_vlan_lock();
    for (vlan_id = SAI_MIN_VLAN_TAG_ID;
         vlan_id <= SAI_MAX_VLAN_TAG_ID; vlan_id++) {
        if(sai_is_internal_vlan_id(vlan_id)) {
            SAI_VLAN_LOG_INFO("Not deleting internal vlan id %d", vlan_id);
            continue;
        }
        if(sai_is_vlan_created(vlan_id)) {
            SAI_VLAN_LOG_TRACE("Removing vlan id %d",vlan_id);
            sai_l2_delete_vlan(vlan_id);
        }
    }
    sai_vlan_unlock();
    return SAI_STATUS_SUCCESS;
    ...
    do {
        ...
        for(num_port = 0; num_port < port_count; num_port++) {
            ...
            if(fwd_mode == SAI_PORT_FWD_MODE_ROUTING) {
                SAI_VLAN_LOG_WARN("port 0x%"PRIx64" in routing mode. \
                                 Cannot add to vlan:%d",
                                           ...
                                           const sai_attribute_t *attr)
{
    ...
    switch(attr->id)
    {
        ...
        case SAI_VLAN_ATTR_STP_INSTANCE:
            SAI_VLAN_LOG_TRACE("Setting stp instance for vlan:%d value=%lu",
                                                       ...
                                                       sai_port_event_notification_t *data)
{
    ...
    for(port_idx = 0; port_idx < count; port_idx++) {
        ...
        if(port_event == SAI_PORT_EVENT_ADD) {
            return sai_vlan_port_default_init(SAI_INIT_DEFAULT_VLAN, port_id);
            ...
            if(sai_is_valid_vlan_port_member(SAI_INIT_DEFAULT_VLAN,
                ...
                return sai_l2_remove_ports_from_vlan(SAI_INIT_DEFAULT_VLAN,
                                                     1, &vlan_port);
            }
        }
    }
