...
/*
 * LAG add port and remove port
 */
TEST_F(lagInit, lag_port_test)
{
    sai_object_id_t lag_id = 0;
    sai_object_id_t lag_id_2 = 0;
    sai_object_list_t lag_port_list;
    sai_object_id_t lag_port[2];
    sai_object_id_t invalid_lag_port[2];


    ASSERT_EQ(SAI_STATUS_SUCCESS,
              sai_lag_api_table->create_lag(&lag_id, 0, NULL));

    memset(lag_port, 0, sizeof(lag_port));
    memset(&lag_port_list, 0, sizeof(lag_port_list));

    lag_port[0] = port_id_1;
    lag_port[1] = port_id_2;

    lag_port_list.count = 2;
    lag_port_list.list = lag_port;

    ASSERT_EQ(SAI_STATUS_SUCCESS,
              sai_lag_api_table->add_ports_to_lag(lag_id,
                                                  &lag_port_list));
    ...
    ASSERT_EQ(SAI_STATUS_INVALID_PORT_MEMBER,
              sai_lag_api_table->add_ports_to_lag(lag_id_2,
                                                  &lag_port_list));

    ASSERT_EQ(SAI_STATUS_SUCCESS,
              sai_lag_api_table->remove_lag(lag_id_2));

    invalid_lag_port[0] = port_id_1;
    invalid_lag_port[1] = port_id_invalid;

    lag_port_list.list = invalid_lag_port;

    ASSERT_EQ(SAI_STATUS_INVALID_PARAMETER,
              sai_lag_api_table->add_ports_to_lag(lag_id, &lag_port_list));

    ASSERT_EQ(SAI_STATUS_INVALID_PARAMETER,
              sai_lag_api_table->remove_ports_from_lag(lag_id, &lag_port_list));

    lag_port_list.list = lag_port;
    ASSERT_EQ(SAI_STATUS_SUCCESS,
              sai_lag_api_table->remove_ports_from_lag(lag_id,
                                                       &lag_port_list));
...
TEST_F(lagInit, lag_attribute_test)
{
    ...
    ASSERT_EQ(SAI_STATUS_SUCCESS,
              sai_lag_api_table->set_lag_attribute(lag_id, (const sai_attribute_t*)&attr));
