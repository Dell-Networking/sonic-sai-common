...
{
    sai_vlan_stat_counter_t counter_ids[] =
    ...
    unsigned int num_counter = sizeof(counter_ids)/sizeof(sai_vlan_stat_counter_t);
    ...
    for(idx = 0; idx < num_counter; idx++) {
        ret_val = sai_vlan_api_table->clear_vlan_stats(SAI_GTEST_VLAN,
                                                      (const sai_vlan_stat_counter_t*)&counter_ids[idx],1);
        if(ret_val == SAI_STATUS_SUCCESS) {
